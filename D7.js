...
const numeriCasuali = generaArrayCasuale();
...
const num = [4, 5, 8, 9, 22, 33, 44];
const numeriPari = filtraNumeriPari(num);
...
*/
const valori = [1, 3, 4, 5];
const incremento = 3;
const valoriIncrementati = incrementaArray(valori, incremento);
function incrementaArray(array, n) {
  return array.map(function (elemento) {
    return elemento + n;
  });
}
console.log(valoriIncrementati);
...
function lunghezzeStringhe(array) {
  return array.map(function (stringa) {
...
const filmMillennioCorrente = filtraFilmMillennioCorrente(movies);
